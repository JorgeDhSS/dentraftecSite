...
if (!empty($_POST))
{
	...
	if($_POST["userName"] == "UserRH" || $_POST["userName"] == "UserFinza")
	{
		try
		{
		...
		$sth = $dbh->prepare('SELECT email FROM users WHERE Nombre = :userName AND password = :pass');
		//$result = $sth->execute([mysqli_real_escape_string($dbh,$_POST["mail"]), mysqli_real_escape_string($dbh,$_POST["password"])]);
    ...
    {
    	echo "Por favor, agregue todos los datos";
    }
}
?>
<head>
      ...
      <div class="xl:ml-20 xl:w-5/12 lg:w-5/12 md:w-8/12 mb-12 md:mb-0">
        <form action="" method="POST"> 
          <div class="flex flex-row items-center justify-center lg:justify-start">
            <p class="text-lg mb-0 mr-4">Visita nuestras páginas</p>
            <a
              href="/redirect?socialNet=1"
          ...
          >
            <p class="text-center font-semibold mx-4 mb-0">Inicia sesión</p>
          </div>
          <!-- Email input -->
          <div class="mb-6">
            <input
